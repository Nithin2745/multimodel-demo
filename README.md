...
For deployment, set `VITE_API_BASE_URL` to the deployed backend URL in the frontend service environment. Set `FRONTEND_ORIGINS` on the backend to the deployed frontend URL. `GEMINI_MODELS` defaults to `gemini-2.5-flash,gemini-2.5-flash-lite` and tries the lighter model if the primary model is busy.
